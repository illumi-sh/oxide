...
use super::{DeserializePacket, SerializePacket};
    ...
    fn serialize(&self, buf: &mut impl Write) -> io::Result<usize> {
        ...
        match self {
            IpcRequest::SyncProject { root_dir } => {
                // Write the variable length message string, preceded by its length
...
impl DeserializePacket for IpcRequest {
    ...
    fn deserialize(mut buf: &mut impl Read) -> io::Result<IpcRequest> {
        ...
        match buf.read_u8()? {
            // Echo
            1 => Ok(IpcRequest::SyncProject { root_dir: Self::extract_string(&mut buf)? }),
            2 => Ok(IpcRequest::InstallProject {
                project_dir: Self::extract_string(&mut buf)?,
